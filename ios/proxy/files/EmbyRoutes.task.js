/**
 ------------------------
 QuantumultX Task脚本配置:
 ------------------------
 [task_local]
 # Emby线路优选-任务 （请修改cron表达式控制想要运行的时间，示例为每天18点运行）
 0 18 * * * https://raw.githubusercontent.com/yyuueexxiinngg/some-scripts/master/ios/proxy/files/EmbyRoutes.task.js
...
async function embyRoutesTaskMain() {
    ...
    for (let i = 0; i < config["embyRoutes"].length; i++) {
        ...
        // 请求可能出错，不使用Promise.race()
        const results = await Promise.all(emby["urls"].map(url => embyRouteTest(url)));
        results.sort((a, b) => a["time"] - b["time"]);

        env.log(`-------------${emby["name"]}----------------`)
        env.log(`${emby["name"]} 检测结束，路线情况：`);
...
function embyRouteTest(url) {
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            env.log(`${urlObj.hostname} 请求超时 ${timeOut}ms`);
            result.msg = `请求超时，请检查网络`;
            resolve(result);
        }, timeOut);
        env.get(requestOps, (error, response, data) => {
            if (error) {
                env.log(`${urlObj.hostname} 请求失败`, JSON.stringify(error));
                resolve(result);
            } else {
                if (data.indexOf('<div class="app-splash"></div>') > -1) {
                    const endTime = new Date().getTime()
                    const costTime = (endTime - startTime) / 1000
                    env.log(`${urlObj.hostname} 请求成功，历时${costTime}s`);
                    result.ok = true;
                    result.time = costTime;
                    resolve(result);
                } else {
                    env.log(`${urlObj.hostname} 请求失败`, JSON.stringify(error));
                    resolve(result);
